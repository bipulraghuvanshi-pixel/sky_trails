...
class AnimatedPlane extends StatefulWidget {
  ...
  const AnimatedPlane({super.key, this.onTap});
...
class _AnimatedPlaneState extends State<AnimatedPlane>
  ...
  void initState() {
    ...
    _flightAnimation = Tween<double>(begin: 0, end: 600).animate(
    ...
    _rotationAnimation = Tween<double>(begin: 0, end: -0.08).animate(
      CurvedAnimation(parent: _flightController, curve: Curves.easeOut),
    );
  }
  ...
  Widget build(BuildContext context) {
    return GestureDetector(
      ...
      child: AnimatedBuilder(
        animation: Listenable.merge([_floatAnimation, _flightController]),

        builder: (context, child) {
          final double x = isFlying ? _flightAnimation.value : 0;

          final double y = isFlying ? 0 : _floatAnimation.value;

          final double rotation = isFlying ? _rotationAnimation.value : 0;

          return Transform.translate(
            offset: Offset(x, y),

            child: Transform.rotate(
              // Both elements share the icon center as their transform origin.
              ...
              child: Stack(
                ...
                children: [
                  if (isFlying)
                    Positioned(
                      // The painter ends at its right edge: (-120 + 146, 14 + 12)
                      // is the aircraft icon's center (26, 26).
                      left: -120,
                      top: 14,
                      child: SizedBox(
                        width: 146,
